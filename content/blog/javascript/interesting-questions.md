...
19. Was hat es mit `scripts/test/mocks` auf sich?
...
27. Explain the whole version management please. Running `code-push` Android without `--targetBinaryVersion` flag gives error `[Error]  No property named "getVersionName()" exists in the "android/gradle.properties" file.`
...
45. Selectors: Why are they wrapped in a function? e.g. (In the example in the boilerplate docs they are not wrapped)
	...
	* Also in `HomePage/selectors.js`: Why is selectHome again wrapped into an *empty* function, if it is directly called thereafter in `selectUsername` ??

4. In `redirectToLogin` hook and in homePath logic in `routes.js`:
...
7. In `app/app.js`, how does this `Promise` work?

	```js
	let storeReady;
	const storeReadyPromise = new Promise((resolve) => { storeReady = resolve; });
	```

	**ANSWER** Speicher storyReadypromise resolve funktion.

3. Was macht diese Zeile anderes als `{children}` z.B. in `Contentcontainer/index.js`?
...
10. In `MultiLocaleContent/index.js`
...
12. Zum `redux-persist`. Wie genau funktioniert das? Sucht das dann automatisch erstmal im Browser, ob da noch etwas abgespeichert ist beim mounten vom redux store?
...
5. Investigate: How does `classNames` from `import classNames from 'classnames';` work?

	**Answer**: Dynamically add `classnames` based on boolean expressions.

8. In `ObjectPage/reducer.js`:
...
2. Where does prop `location` come from? i.e. how is passed to the containers from the router?
...
5. Proxipedia: In `NavigationList.js`, where does the parameter `scrollProps` come from in:
...
5. in `src/common/localstorage/index.js`
...
6. How does the following export of common button styles function work?
...
12. How does storage work?

	--> look at `util/fs` files

14. in `ListItemVideo.js`:
...
15. According to React documentation `this.props.children` can be either an array or an object if there's only one single child. Is `{this.props.children}` robust against that?

	???

16. In `LightBoxTrigger.js`:
...
17. `measure` function in `LightBoxTrigger.js`:
...
18. Navigation e.g. `container/Home.js`. What do functions `right` and `left` do?

	See `App.js` --> render whatever is in right or left part.

19. What does the action `navigateReset()` do?

	resets the navigation.

20. How is `modalState` in `App.js` connected to `modalStateReducer`?
...
21. How could I get information about the current POI inside an action creator which I build like `actions/menu.js`? i.e. `story.js` in commit `bcfb7f86e2ed4c21435353dd1821c2838668ef82`

22. Why start with return statement within action creator with `Promise.resolve()`?
...
23. Javascript Closures - debounce
...
27. Why do you use the `XMLHttpRequest();` in the `upload` function in `Api.js` and not `fetch`?

	`fetch` hat keinen `onprogress` callback to show progress.

28. Why did you create an Api **class**?
...
29. Error when running `npm run debug:android` with Genymotion:
...
30. In `CameraActionContainer`, why don't you define the following `onAction` as a redux thunk?:
...
31. In `FeedItem.js`, warum sollte es in der folgenden Zeile jemals kein `onImage` geben? Wird doch immer weitergegeben..
...
32. In `NavigationStateUtils.js` of `NavigationExperimental`: Do you know why in the `pop` function in `index: lastChildren.length - 2,` the index is reduced by 2?

	one child: index = 0, two children: index = 1.

33. Navigation: How is the correct image accessed?
...
34. In general: What is the difference between `push` and `jumpToIndex` in routing? Why do I need the later anyway?

	we use `navigateReset` to reset the stack.
	e.g. `Favorites` becomes index 0 when clicking Favorites from Home in Proxipedia 1.5

35. In constructor of React class: `this.state = { ... }` vs. `state = { ... }`? The same? Why can I actually call `this.state` in a stateless component??

	`state` is an instance variable.
	--> Try: setState() sollte in stateless component nicht funktionieren...

36. Explain syntax of stateless component. It is not a class, so it does not need a constructor for props.. But how does it work?
...
37. Javascript: Two different ways of defining properties of objects which are then called with the dot `.` operator: `someArray.length` vs. e.g. `Animated.timing(...).start()`:
...
18. **Fastlane**:
	...
	* Wie ruft `apply_flavor` `scripts/flavor` und `scripts/flavor_copy` auf?
		* --> FOUND IT: In `fastlane/actions/apply_flavor.rb`
...
23. What do the npm scripts `bundle:ios` and `bundle:android` do?

	--> Old scripts to bundle the code --> Now done by `fastlane` and in `Jenkins`

24. Why do `deploy:ios` and `deploy:android` work as `fastlane ios` and `fastlane android`?

	--> Probably defaults

40. `PP 1.5 App` - State is initialized as a `Map` in `poisReducer.js`. So why can you then merge it with a `Seq`? Why don't you initialize it as a `Seq` in the first place?
...
42. How do I delete the `webpack` cached offline files in Chrome Dev Tools?

	* DevTools/Application/Clear Storage

43. Why do we have this in the reducers?
...
21. Martin, how do you use the tool `https://github.com/wix/wml` when creating a RN plugin?
...
5. What does `import { shallow } from 'enzyme';` the `shallow` function do?
...
3. What is `util/react/coalesceNonElementChildren.js`?

	--> Martin

4. What does `_computeSyntheticState` in `components/Button/Button.js` do?
